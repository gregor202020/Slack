...
export default function () {
  ...
  if (scenario === 'messages') {
    group('Read and send messages', () => {
      ...
      // Send a message ~10% of the time under stress
      if (Math.random() < 0.1) {
        const sendRes = http.post(
          `${BASE_URL}/api/messages/channel/${channelId}`,
          JSON.stringify({
            body: 'Stress test message from k6 at ' + new Date().toISOString(),
          }),
          authHeaders()
        )
        check(sendRes, {
  ...
  sleep(Math.random() * 2 + 0.5) // 0.5-2.5 seconds think time (faster under stress)
}
